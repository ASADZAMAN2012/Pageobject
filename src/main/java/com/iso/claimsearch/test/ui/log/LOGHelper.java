package com.iso.claimsearch.test.ui.log;
...
/**
 * @author i91525 (Arthi Agoramoorthi)
...
public class LOGHelper extends HTMLLayout {
	...
	String format(LoggingEvent event) {
		...
		else if(event.getLevel().equals(Level.INFO)&&event.getMessage().toString().toUpperCase().contains("PASSED")) {
			sbuf.append("<font color=\"GREEN\"><strong>");
			sbuf.append(Transform.escapeTags(String.valueOf(event.getLevel())));
			sbuf.append("</strong></font>");
		} 
		else if(event.getLevel().equals(Level.INFO)&&event.getMessage().toString().toUpperCase().contains("FAILED")) {
		...
		if(event.getLevel().equals(Level.INFO)&& event.getRenderedMessage().toString().toUpperCase().contains("PASSED")) {
			sbuf.append("<font color=\"GREEN\"><strong>");
			sbuf.append(escapedLogger);
			sbuf.append("</strong></font>");
		} 
		else if(event.getLevel().equals(Level.INFO)&&event.getRenderedMessage().toString().toUpperCase().contains("FAILED")) {
			sbuf.append("<font color=\"RED\"><strong>");
			sbuf.append(escapedLogger);
			sbuf.append("</strong></font>");
		} 
